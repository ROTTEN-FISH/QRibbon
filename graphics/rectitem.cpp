...
void rectItem::setRectSize(QRectF mrect,bool fixed, bool bResetRotateCenter)
{
    f=fixed;
    m_oldRect = mrect;
    ...
    //m_leftRectf = QRectF(m_oldRect.x()-5,m_oldRect.y()+5,10,m_oldRect.height()-10);
    //m_leftPolygon = getRotatePolygonFromRect(m_RotateCenter,m_leftRectf,m_RotateAngle);

    //m_topRectf = QRectF(m_oldRect.x()+5,m_oldRect.y()-5,m_oldRect.width()-10,10);
    //m_topPolygon = getRotatePolygonFromRect(m_RotateCenter,m_topRectf,m_RotateAngle);

    m_rightRectf = QRectF(m_oldRect.right()-4,m_oldRect.y()+4,8,m_oldRect.height()-8);
    m_rightPolygon = getRotatePolygonFromRect(m_RotateCenter,m_rightRectf,m_RotateAngle);

    m_bottomRectf = QRectF(m_oldRect.x()+4,m_oldRect.bottom()-4,m_oldRect.width()-8,8);
...
void rectItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    //qDebug()<<m_oldRectPolygon[0]<<m_oldRectPolygon[1]<<m_oldRectPolygon[2]<<m_oldRectPolygon[3];
    QRectF rect = QRectF(pf.x()-8,pf.y()-8,16,16);
...
void rectItem::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if(r){
        emit mysignals(1,pos(),m_oldRectPolygon[0],m_oldRectPolygon[1],m_oldRectPolygon[2],m_oldRectPolygon[3],a);
    }else{
        emit mysignals(1,pos(),m_oldRectPolygon[0],m_oldRectPolygon[1],m_oldRectPolygon[2],m_oldRectPolygon[3],-1);
    }
}
...
void rectItem::SetRotate(qreal RotateAngle, QPointF ptCenter)
{
    m_bRotate = true;
...
QPointF rectItem::getRotatePoint(QPointF ptCenter, QPointF ptIn, qreal angle)
{
...
QList<QPointF> rectItem::getRotatePoints(QPointF ptCenter, QList<QPointF> ptIns, qreal angle)
{
    QList<QPointF> lstPt;
...
QPolygonF rectItem::getRotatePolygonFromRect(QPointF ptCenter, QRectF rectIn, qreal angle)
{
    QVector<QPointF> vpt;
