...
namespace YZVision
{
	class Cutcloud_3D :public Basic3D
	{
	public: Cutcloud_3D();
		  ~Cutcloud_3D();
		  ...
		  */
		  uint32_t cutcloud(InputParam inputParam, OutputParam* outputParam);

	private:
		uint32_t setInputCloud(vPointsXYZ* _cloud);
		uint32_t Chooseroi(vPointsXYZ& pointCloudROI,bool x_0);
		uint32_t CutCloudroi(vPointsXYZ* inputcloud, bool x_0,PointXYZ m_StartPoint, PointXYZ m_EndPoint, vPointsXYZ* outputcloud);
