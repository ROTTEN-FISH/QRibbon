...
File name:	 passthrough_filter_3D.h
Description: 提供点云直通滤波的算子.
Note:	     内部资料，请勿外传.
...
namespace YZVision
{
	class PassthroughFilter_3D
	{
	public: PassthroughFilter_3D();
		  ~PassthroughFilter_3D();
		  ...
		  /*
			  ...
			  */
		  uint32_t passThroughFilter(InputParam inputParam, OutputParam* outputParam);

	private:
		uint32_t Passthroughcloud(c_vPointsXYZ* inputcloud, int a, bool R_0, float x_0, float x_1, vPointsXYZ* outputcloud);
