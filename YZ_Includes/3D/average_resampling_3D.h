...
File name:	 average_resampling_3D.h
Description: 提供点云均匀采样的算子.
Note:	     内部资料，请勿外传.
...
namespace YZVision
{
	class Averageresampling_3D
	{
	public: Averageresampling_3D();
		  ~Averageresampling_3D();
		  ...
		  /*
			  ...
			  */
		  uint32_t averageresampling(InputParam inputParam, OutputParam* outputParam);

	private:
		uint32_t AverageResampling(c_vPointsXYZ* inputcloud, float r_0, vPointsXYZ* outputcloud);
