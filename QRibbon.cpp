    ...
    , _(new QRibbonPrivate(this))
{
    ...
    styleActionGroup->addAction(_styleMenu->addAction("默认", [&]()
    {
        setColor("rgb(86, 102, 204)");
    }));
...
bool QRibbon::eventFilter(QObject *tgt, QEvent *e)
{
    ...
    {
//        QIcon icon;
//        if(_->_mainWindow->windowState() & (Qt::WindowFullScreen | Qt::WindowMaximized))
//            icon.addFile(tr(":/Resource/icon/normal.png"));
//        else
//            icon.addFile(tr(":/Resource/icon/max.png"));
//        ui->pushButtonFullScreen->setIcon(icon);
//ui->pushButtonFullScreen->setText(_->_mainWindow->windowState() & (Qt::WindowFullScreen | Qt::WindowMaximized)?"^":"x");
    }
...
void QRibbon::on_pushButtonMaxTab_clicked()
{
    if(m_bExpandStaus == false)
    {
        expandTab();
